...
class AndroidPlaybackController(
    ...
    fun VideoSurface(
        ...
        gestures: LiveVideoGestures? = null,
    ) {
        ...
        BoxWithConstraints(modifier = modifier.background(WukkiColors.video)) {
            ...
            val playerModifier =
                targetAspect?.let { target ->
                    if (hostAspect > target) {
                        Modifier.fillMaxHeight().aspectRatio(target)
                    } else {
                        Modifier.fillMaxWidth().aspectRatio(target)
                    }
    ...
    private fun installLiveTouchHandler(
        view: PlayerView,
        gestures: LiveVideoGestures?,
    ) {
        if (gestures == null) {
    ...
    private fun createPlayer(
        ...
        val loadControl =
            DefaultLoadControl
                ...
                .build()
        player =
            ExoPlayer.Builder(context).setLoadControl(loadControl).build().also { mediaPlayer ->
    ...
    private fun AspectRatioMode.resizeMode() =
        when (this) {
    ...
    private fun AspectRatioMode.targetAspectRatio(): Float? =
        when (this) {
