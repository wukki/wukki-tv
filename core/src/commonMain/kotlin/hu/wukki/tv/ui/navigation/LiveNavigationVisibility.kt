...
sealed interface LiveNavigationVisibilityEvent {
    ...
    data class Reveal(
        val focusNavigation: Boolean,
    ) : LiveNavigationVisibilityEvent
...
fun LiveNavigationVisibilityState.reduce(event: LiveNavigationVisibilityEvent): LiveNavigationVisibilityResult =
    when (event) {
        LiveNavigationVisibilityEvent.EnterLive,
        LiveNavigationVisibilityEvent.Interact,
        -> {
            LiveNavigationVisibilityResult(
                copy(visible = true, interactionSequence = interactionSequence + 1),
            )
        }

        is LiveNavigationVisibilityEvent.Reveal -> {
            LiveNavigationVisibilityResult(
                copy(visible = true, interactionSequence = interactionSequence + 1),
                if (event.focusNavigation) {
        ...
        LiveNavigationVisibilityEvent.Timeout -> {
            LiveNavigationVisibilityResult(
                copy(visible = false),
                if (visible) {
        ...
        LiveNavigationVisibilityEvent.LeaveLive -> {
            LiveNavigationVisibilityResult(copy(visible = false))
        }
    }
