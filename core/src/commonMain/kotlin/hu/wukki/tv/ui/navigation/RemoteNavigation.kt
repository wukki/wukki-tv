...
fun AppBackNavigationState.reduce(): AppBackNavigationEffect =
    when {
...
fun MainMenuNavigationState.reduce(
    key: RemoteKey,
    itemCount: Int,
): MainMenuNavigationResult {
...
fun ChannelNavigationState.reduce(
    ...
    return when (safe.focus) {
        ChannelRemoteFocus.FILTERS -> {
            when (key) {
                RemoteKey.LEFT -> {
                    if (safe.filterIndex == 0) {
                        ChannelNavigationResult(safe, ChannelNavigationEffect.ExitToMainMenu)
                    } else {
                        ChannelNavigationResult(safe.copy(filterIndex = safe.filterIndex - 1))
                    }
                }

                RemoteKey.RIGHT -> {
                    if (safe.filterIndex >= lastFilter) {
                ...
                RemoteKey.DOWN -> {
                    ChannelNavigationResult(safe.copy(focus = ChannelRemoteFocus.LIST))
                }

                RemoteKey.CONFIRM -> {
                    ChannelNavigationResult(safe, ChannelNavigationEffect.ActivateFilter(safe.filterIndex))
                }
        ...
        ChannelRemoteFocus.SEARCH -> {
            when (key) {
                RemoteKey.LEFT, RemoteKey.RIGHT, RemoteKey.DOWN -> {
                    if (searchHasText) {
                        ChannelNavigationResult(safe, handled = false)
                    } else {
        ...
        ChannelRemoteFocus.LIST -> {
            when (key) {
                ...
                RemoteKey.UP -> {
                    if (safe.channelIndex == 0) {
                        ChannelNavigationResult(safe.copy(focus = ChannelRemoteFocus.FILTERS))
                    } else {
                        ChannelNavigationResult(safe.copy(channelIndex = safe.channelIndex - 1))
                    }
                }

                RemoteKey.DOWN -> {
                    ChannelNavigationResult(safe.copy(channelIndex = (safe.channelIndex + 1).coerceAtMost(lastChannel)))
                }
        ...
        ChannelRemoteFocus.FAVORITE -> {
            when (key) {
                ...
                RemoteKey.CONFIRM -> {
                    ChannelNavigationResult(safe, ChannelNavigationEffect.ToggleFavorite(safe.channelIndex))
                }
...
fun SettingsSection.options(): List<SettingsOptionId> =
    when (this) {
...
sealed interface SettingsNavigationEffect {
    ...
    data class Activate(
        val option: SettingsOptionId,
    ) : SettingsNavigationEffect
...
fun SettingsNavigationState.reduce(key: RemoteKey): SettingsNavigationResult {
    ...
    if (section == null) {
        ...
        return when (key) {
            RemoteKey.UP -> {
                SettingsNavigationResult(copy(categoryIndex = (safeCategory - 1).coerceAtLeast(0)))
            }

            RemoteKey.DOWN -> {
                SettingsNavigationResult(copy(categoryIndex = (safeCategory + 1).coerceAtMost(lastCategory)))
            }

            RemoteKey.LEFT -> {
                SettingsNavigationResult(copy(categoryIndex = safeCategory), SettingsNavigationEffect.ExitToMainMenu)
            }
