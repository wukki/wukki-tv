...
class PlaybackController(
    initialLanguage: AppLanguage = AppLanguage.HUNGARIAN,
) : PlaybackEngine {
        ...
        PlaybackSession(
            object : PlaybackAdapter {
                    ...
                    listener =
                        object : MediaPlayerEventAdapter() {
                            ...
                            override fun playing(mediaPlayer: MediaPlayer) {
                                SwingUtilities.invokeLater { session.playing(generation) }
                            }

                            override fun error(mediaPlayer: MediaPlayer) {
                                SwingUtilities.invokeLater { session.failed(generation) }
                            }
                ...
                override fun volume(value: Int) {
                    component?.mediaPlayer()?.audio()?.setVolume(value)
                }
            ...
            PlaybackScheduler { delay, action ->
                val timer =
                    javax.swing.Timer(delay.toInt()) { action() }.apply {
                        isRepeats = false
                        start()
                    }
    ...
    private fun createComponent(): OverlayCallbackMediaPlayerComponent? =
        try {
            if (runtime == null && !NativeDiscovery().discover()) {
                val messageKey =
                    when (runtimeResolution.issue) {
