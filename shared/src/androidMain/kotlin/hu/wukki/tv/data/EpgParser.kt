...
actual object EpgParser {
    ...
    private class ProgrammeHandler(private val programmes: MutableList<Programme>) : DefaultHandler() {
        ...
        override fun startElement(uri: String?, localName: String?, qName: String, attributes: Attributes) {
            when (tagName(localName, qName)) {
                ...
                "title", "desc" -> if (programme != null) { activeTextTag = tagName(localName, qName); text.setLength(0) }
        ...
        override fun endElement(uri: String?, localName: String?, qName: String) {
            ...
            when (tag) {
                "title" -> current?.takeIf { activeTextTag == "title" }?.let { if (it.title.isBlank()) it.title = text.toString().trim(); activeTextTag = null }
                "desc" -> current?.takeIf { activeTextTag == "desc" }?.let { if (it.description == null) it.description = text.toString().trim().ifBlank { null }; activeTextTag = null }
                "icon", "image" -> current?.takeIf { activeTextTag == tag }?.let { it.imageUrl = it.imageUrl ?: validImageUrl(text.toString()); activeTextTag = null }
                "programme" -> {
                    programme = null
                    activeTextTag = null
                    if (current != null && current.channelId.isNotBlank() && current.start != null && current.end != null && current.end > current.start) {
                        programmes += Programme(current.channelId, current.title, current.start, current.end, current.description, current.imageUrl)
                    }
                }
            }
        }
    ...
    private data class MutableProgramme(
        val channelId: String, val start: Long?, val end: Long?, var title: String = "", var description: String? = null, var imageUrl: String? = null
    )

    private fun validImageUrl(value: String?): String? = value?.trim()?.replace("&amp;", "&")
