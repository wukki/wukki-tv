...
internal class AppSessionController(
    ...
    fun handleLiveNavigation(event: LiveNavigationVisibilityEvent) {
        with(session) {
            ...
            when (result.effect) {
                LiveNavigationVisibilityEffect.NONE -> {
                    Unit
                }
    ...
    fun handleLiveChannelPreview(event: LiveChannelPreviewEvent): Boolean {
        return with(session) {
            val result =
                liveChannelPreviewState.reduce(
                    event = event,
            ...
            when (result.effect) {
                LiveChannelPreviewEffect.NONE -> {
                    if (result.handled) overlayRequest++
                }
                ...
                LiveChannelPreviewEffect.OPEN_CHANNEL -> {
                    result.channelIdToOpen?.let { channelId ->
        ...
        with(session) {
            ...
            when (transition.effect) {
                ...
                GuideProgrammeDialogEffect.OPEN_CHANNEL -> {
                    guideState.focusedProgramme(guideDataSource, guideTimeline(tick, model.guideLatestProgrammeEnd()))?.first?.let { channel ->
    ...
    fun applySettingsEffect(effect: SettingsNavigationEffect) {
        with(session) {
            when (effect) {
                SettingsNavigationEffect.None -> {
                    Unit
                }

                SettingsNavigationEffect.ExitToMainMenu -> {
                    focusZone = TvFocusZone.MAIN_NAVIGATION
                }

                is SettingsNavigationEffect.Adjust -> {
                    model.adjustSetting(effect.option, effect.delta)
                }

                is SettingsNavigationEffect.Activate -> {
                    when (val option = effect.option) {
                        PlaybackSettingsOption.AUTOPLAY -> {
                            model.adjustSetting(option, 1)
                        }
                        ...
                        PlaybackSettingsOption.RECONNECT -> {
                            model.adjustSetting(option, 1)
                        }

                        PlaybackSettingsOption.RETRIES -> {
                            model.adjustSetting(option, 1)
                        }

                        PlaybackSettingsOption.VOLUME -> {
                            Unit
                        }

                        is DisplaySettingsOption -> {
                            model.adjustSetting(option, 1)
                        }
                        ...
                        PlaylistSettingsOption.SCHEDULE -> {
                            model.adjustSetting(option, 1)
                        }

                        PlaylistSettingsOption.REFRESH -> {
                            scope.launch { model.refreshOfficialPlaylist() }
                        }

                        is AboutSettingsOption -> {
                            if (
        ...
        return with(session) {
            val result =
                AppRemoteState(
                    ...
                    focus = focusZone,
                    menuIndex = mainNavigationIndex,
                    exitConfirmation = exitConfirmationState,
                    requireDoubleBack = requireDoubleBackToExit,
                    ...
                    dialogVisible = guideProgrammeDetailsVisible,
                    overlayVisible = programmeOverlayVisible,
                    preview = liveChannelPreviewState,
                    number = channelNumberInput,
                    navigationVisible = liveNavigationState.visible,
                ).reduce(key)
            ...
            result.effects.forEach { effect ->
                when (effect) {
                    ...
                    is AppRemoteEffect.Dialog -> {
                        handleGuideProgrammeDialogEvent(effect.event)
                    }

                    is AppRemoteEffect.Back -> {
                        handled = applyBackEffect(effect.effect)
                    }
                    ...
                    is AppRemoteEffect.SwitchChannel -> {
                        switchLiveChannel(effect.delta)
                    }
                    ...
                    is AppRemoteEffect.GuideKey -> {
                        guideState.handleRemoteKey(effect.key, guideDataSource, scope, guideTimeline(tick, model.guideLatestProgrammeEnd()))
                    }

                    is AppRemoteEffect.SelectNumber -> {
                        if (model.selectChannelByNumber(effect.number)) overlayRequest++
                    }
                    ...
                    is AppRemoteEffect.Preview -> {
                        when (effect.result.effect) {
                            LiveChannelPreviewEffect.NONE -> {
                                if (effect.result.handled) overlayRequest++
                            }
                            ...
                            LiveChannelPreviewEffect.OPEN_CHANNEL -> {
                                effect.result.channelIdToOpen?.let { id ->
                    ...
                    is AppRemoteEffect.Settings -> {
                        applySettingsEffect(effect.effect)
                    }
                    ...
                    is AppRemoteEffect.Channels -> {
                        when (val action = effect.effect) {
                            ...
                            is ChannelNavigationEffect.OpenChannel -> {
                                visibleChannels.getOrNull(action.index)?.let { openChannelFromBrowser(it.id) }
                            }

                            is ChannelNavigationEffect.ToggleFavorite -> {
                                visibleChannels.getOrNull(action.index)?.let { model.toggleFavorite(it.id) }
                            }
                        }
                    }
                }
        ...
        with(session) {
            DashboardCallbacks(
                ...
                onSettingsSectionChange = { section ->
                    val categoryIndex =
                        section?.let { SettingsSection.entries.indexOf(it).coerceAtLeast(0) }
