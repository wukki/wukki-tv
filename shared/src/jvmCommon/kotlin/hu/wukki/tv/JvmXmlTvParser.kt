...
object JvmXmlTvParser : XmlTvParser {
    override fun parse(xml: String): List<Programme> {
        ...
        return parseInput(InputSource(StringReader(xml)))
    }

    override fun parse(content: RemoteContent): List<Programme> = parseInput(InputSource(RemoteContentInputStream(content)))

    private fun parseInput(input: InputSource): List<Programme> {
        ...
        try {
            ...
            val offset =
                raw
                    ...
                    .ifBlank { "+0000" }
            OffsetDateTime
                .parse("$base $offset", DateTimeFormatter.ofPattern("yyyyMMddHHmmss Z"))
                .toInstant()
                .toEpochMilli()
        } catch (_: Exception) {
            runCatching {
                LocalDateTime
                    .parse(raw.trim().take(14), DateTimeFormatter.ofPattern("yyyyMMddHHmmss"))
    ...
    private class ProgrammeHandler(
        private val programmes: MutableList<Programme>,
    ) : DefaultHandler() {
        ...
        override fun endElement(
            ...
            when (tag) {
                "title" -> {
                    current?.takeIf { activeTextTag == "title" }?.let {
                ...
                "desc" -> {
                    current?.takeIf { activeTextTag == "desc" }?.let {
                ...
                "icon", "image" -> {
                    current?.takeIf { activeTextTag == tag }?.let {
    ...
    private fun validImageUrl(value: String?): String? =
        value
            ?.trim()
            ?.replace("&amp;", "&")
...
private class RemoteContentInputStream(
    private val content: RemoteContent,
) : InputStream() {
    ...
    override fun read(
        ...
        offset: Int,
        length: Int,
    ): Int = content.read(buffer, offset, length)

    // RemoteContent lifetime belongs to the caller that opened it.
