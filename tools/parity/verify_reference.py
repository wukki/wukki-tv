...
def source_paths():
    ...
    return [str(p.relative_to(ROOT)) for p in sorted(paths)]
...
def pinned_sources(names):
    return {
        name: digest(subprocess.check_output(['git', 'show', f'{COMMIT}:{name}'], cwd=ROOT))
        for name in names
    }
...
def main():
    ...
    if args.record:
        reviewed_sources = current_sources()
        ...
        manifest = {
            ...
            'sources': reviewed_sources, 'artifacts': artifacts(),
        }
        path.write_text(json.dumps(manifest, ensure_ascii=False, indent=2) + '\n')
    manifest = json.loads(path.read_text())
    assert manifest['referenceCommit'] == COMMIT
    assert manifest['sources'] == pinned_sources(manifest['sources']), 'Pinned reference sources no longer match their recorded hashes'
